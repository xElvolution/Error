...
const Updater: React.FC<{ chainId: number }> = ({ chainId }) => {
        ...
        const getTransaction = async () => {
          ...
          poll(
            async () => {
              ...
              // Show confetti when the transaction is successful
              if (receipt.status === 1) {
  ...
  const nonBscFarmPendingTxns = useSWRImmutable(
    chainId && ['nonBscFarmPendingTxns', transactions],
    () =>
  ...
  useSWRImmutable(
    chainId && Boolean(nonBscFarmPendingTxns?.data?.length) && ['checkNonBscFarmTransaction', FAST_INTERVAL, chainId],
    () => {
      nonBscFarmPendingTxns.data.forEach((hash) => {
